...
def calcule_disk_net_domain_load(time_elapsed,raw_stats_after,raw_stats_before):
    ...
    block_dict['block_r_bytes_per_sec']  = (raw_stats_after['block_r_bytes'] - raw_stats_before['block_r_bytes']) / time_elapsed
    block_dict['block_w_bytes_per_sec']  = (raw_stats_after['block_w_bytes'] - raw_stats_before['block_w_bytes']) / time_elapsed
    block_dict['block_r_reqs_per_sec']  = (raw_stats_after['block_r_reqs'] - raw_stats_before['block_r_reqs']) / time_elapsed
    block_dict['block_w_reqs_per_sec']  = (raw_stats_after['block_w_reqs'] - raw_stats_before['block_w_bytes']) / time_elapsed
    ...
    if try_socket(hostname,port,timeout) is True:
        ...
        try:
            ...
            try:
                ...
                log.debug("host {} with ip {} can connect with ssh without password but the key fingerprint must be incorporated in ~/.ssh/known_hosts".format(hostname,ip))
                ssh.close()
                return False

            except:
                log.error("host {} with ip {} can't connect with ssh without password. Reasons? timeout, ssh authentication with keys is needed, port is correct?".format(hostname,ip))
...
def execute_commands(host,ssh_commands,dict_mode=False,user='root',port=22):
    ...
    update_disk_operation(id,{'time_elapsed':time_elapsed,'host':host,'commands':ssh_commands,'results':array_out_err})
